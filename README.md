...
If you want to use Stable Diffusion, you will first need to register an account on the website [Huggingface](https://huggingface.co/) and agree to the terms and conditions for the model. You can find the version of the model required by going here:

[sd-v1-4.ckpt](https://huggingface.co/CompVis/stable-diffusion-v-1-4-original/blob/main/sd-v1-4.ckpt)

This is the latest model at the time of writing. You may also check if there are newer versions of the weights available.

**DOCKER USERS**:  Put this into a folder named `ldm/stable-diffusion-v1` and rename them `model.ckpt`. Follow the instructions below carefully because SD is not enabled by default.
...
Alternatively, you may also run with some workflows disabled to prevent out-of-memory crashes. To do that, pass one of these environment variables:
```
DISABLE_DALLE_MEGA
DISABLE_GLID3XL
DISABLE_SWINIR
...
```bash
docker run -e DISABLE_GLID3XL='1' -p 51005:51005 -v $HOME/.cache:/home/dalle/.cache --gpus all jinaai/dalle-flow
```
...
**Stable Diffusion may only be enabled if you have downloaded the weights and make them available as a virtual volume while enabling the environmental flag for SD**.
...
```bash
docker run -e ENABLE_STABLE_DIFFUSION="1" -e DISABLE_DALLE_MEGA="1" -e DISABLE_GLID3XL="1" -p 51005:51005 -v YOUR_MODEL_PATH/ldm:/dalle/stable-diffusion/models/ldm/ -v $HOME/.cache:/home/dalle/.cache --gpus all jinaai/dalle-flow
```
